...
I have followed the odds until closing, and indeed the odds for Atletico drifted from 2.28 to 2.7 which is like 6.8%. That is huge. The draw odds stayed almost the same, 3.48. Inter odds also drifted from 3.25 to 2.70. If it is not just noise, then it looks like using secondary market and a main market, could actually produce signal to other main market. But this was just one test case and it could have easily been noise, I will need much more data for that. In the next post I will elaborate why given my assumptions on previous posts, my tendency is to drop this pipeline altoghther.
